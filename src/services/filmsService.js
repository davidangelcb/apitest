import filmsRepository from "../repositories/filmsRepository.js"; // Asegúrate de incluir la extensión .js

import { translateObjectFilm } from "./../utils/misc.js";
...
const filmsService = {
  ...
  getAllClients: async (page, search) => {
    ...
    try {
      ...
      let response = await filmsRepository.getAll(pagination, searchLink);
      ...
      for (let i = 0; i < items.length; i++) {
        items[i] = translateObjectFilm(items[i]); // Imprime cada elemento
      }
  ...
  getClientById: async (id) => {
    try {
      let people = await filmsRepository.getById(id);

      return translateObjectFilm(people.data);
    } catch (error) {
      // manejo de logs
      console.log(error);
      return null;
